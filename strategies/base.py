...
class StrategyBase(bt.Strategy):
    ...
    def short(self):
        ...
        amount = value * 0.99
        self.log(
            "Sell ordered: $%.2f. Amount %.6f %s - $%.2f USDT"
            % (self.data0.close[0], amount, COIN_TARGET, value),
            True,
        )
    ...
    def long(self):
        ...
        self.log(
            "Buy ordered: $%.2f. Amount %.6f %s. Ballance $%.2f USDT"
            % (self.data0.close[0], amount, COIN_TARGET, value),
            True,
        )
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Expired]:
            ...
            self.log(
                "Order Canceled/Margin/Rejected: Status %s - %s"
                % (order.Status[order.status], self.last_operation),
                True,
            )
